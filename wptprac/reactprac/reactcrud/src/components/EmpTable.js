...
componentDidMount(){
    ...
    empservice.getEmployees().then((result)=>{
        console.log(result.data);
        this.setState({...this.state,earr:result.data,serarr:result.data})
    ...
    if(prevState.searchtext!==this.state.searchtext){
      ...
      if(this.state.searchtext!==""){
        let newarr=this.state.earr.filter(emp=>emp.ename.includes(this.state.searchtext))
         this.setState({...this.state,serarr:[...newarr]})
      }else{
          this.setState({...this.state,serarr:this.state.earr})  
      }
    }
...
render(){
  ...
  <table className="table table-striped">
    ...
    <tbody>
      {this.state.serarr.map((emp)=><tr key={emp.empid}>
        <td>{emp.empid}</td>
        <td>{emp.ename}</td>
        <td>{emp.sal}</td>
        <td>
